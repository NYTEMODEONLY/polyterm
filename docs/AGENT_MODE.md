...
After restarting the client, tools are exposed through MCP as `agent.manifest`, `agent.schemas`, `market.search`, `market.resolve`, `market.research`, `archive.search`, `archive.status`, `analytics.arbitrage`, `analytics.thesis`, `wallet.inspect`, and `wallet.whales`.
...
2. Resolve or search for a market when identifiers are ambiguous.
3. Generate a lower-level thesis when you need raw thesis internals.
4. Inspect wallet/whale activity if the thesis depends on smart money.
5. Check cross-venue spreads.
6. Collect snapshots if the market needs observation over time.
7. Create local alert rules only after policy approval.
...
|-------|---------|----------|
| Read-only | Reads APIs or local state only | `market.research`, `archive.search`, `archive.status`, `analytics.thesis`, `market.search`, `wallet.inspect` |
